  ...
  app.use(cors({
    origin: ["https://zeai-hrm011.netlify.app"],
  ...
  // Keep socket connections alive (helps render/free-hosting)
  setInterval(() => {
    io.emit("server-heartbeat", { ts: Date.now() });
  }, 20000); // every 20s
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("call-user", (data) => {
      if (!data || !data.target) return console.warn('⚠ call-user missing target', data);
  ...
  if (!data || !data.to) return console.warn('⚠ answer-call missing "to"', data);
